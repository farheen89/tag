<?php

include_once 'config/General.php';

echo "<font size=\"7\"><strong>Path Configurations<br><br></strong></font>";

echo "<font size=\"5\"><strong>core Path<br><br></strong></font>";

echo "<font size=\"4\"><strong>Diretorio Raiz /: </strong></font>".$ROOTPATH."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/core/\": </strong></font>".$ROOT_CORE."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/core/modules/\": </strong></font>".$PATH_APP_CORE."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/core/lib/\": </strong></font>".$PATH_LIB_CORE."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/core/lib/db/\": </strong></font>".$PATH_DB_CORE."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/core/lib/js/\": </strong></font>".$PATH_JS_CORE."<br><br><br><br>";

echo "<font size=\"5\"><strong>US Path<br><br></strong></font>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/us/\": </strong></font>".$ROOT_US."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/us/layout/\": </strong></font>".$PATH_LAYOUT_US."<br><br>";

echo "<font size=\"4\"><strong>Diretorio da pasta \"/us/layout/css/\": </strong></font>".$PATH_CSS_US."<br><br>";

